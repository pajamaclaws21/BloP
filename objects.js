...
// SF: MOD: remember new category names (they are identical to original category names at startup)
...
SpriteMorph.prototype.initBlocks = function () {
    ...
    */
		...
		// SF: MOD: define unstoppable setup hat whose script must be run at load time
		receiveOnLoad: {
			type: 'hat',
			category: 'control',
			spec: 'when loading and never stop'
		},

		// SF: MOD: define hat whose script must be run before another script is run WHEN CLICKED
		receiveOnRun: {
			type: 'hat',
			category: 'control',
			spec: 'when running another script'
		},
		runOtherScripts: {
			type: 'command',
			category: 'control',
			spec: 'run other scripts'
		},
		runFirstScriptOfSprite: {
			type: 'command',
			category: 'control',
			spec: 'run first script of %spr'
		},
...
SpriteMorph.prototype.blockAlternatives = {
    ...
    // SF: MOD: define alternative blocks for "when load and never stop" block
    receiveOnLoad: ['receiveGo', 'receiveOnRun'],
    // SF: MOD: define alternative blocks for "when run another script" block
    receiveOnRun: ['receiveGo', 'receiveOnLoad'],
...
SpriteMorph.prototype.blockTemplates = function (category) {
    ...
    */
		...
		// SF: MOD: add "when load and never stop" block to Control category
        blocks.push('-');
        blocks.push(block('receiveOnLoad'));
		// SF: MOD: add "when runnin another script", "run other scripts" and "run first script of sprite" blocks to Control category
        blocks.push(block('receiveOnRun'));
        blocks.push(block('runOtherScripts'));
        blocks.push(block('runFirstScriptOfSprite'));
        blocks.push('-');
...
// SF: MOD (start): hide "create" buttons if GUI is locked
...
// SF: MOD (end): hide buttons if GUI is locked
}
...
// SF: MOD (start): hide buttons if GUI is locked
if (!ide.isLocked) {
...
// SF: MOD: hide buttons if GUI is locked
}
    ...
    palette.userMenu = function () {
        ...
        function hasHiddenPrimitives() {
            ...
            return Object.keys(hiddens).some(function (any) {
				// SF: MOD: takes into account possible not existing blocks (eg. stopAll)
                // return defs[any].category === category ||
                return (defs[any] && defs[any].category === category) ||
...
// SF: MOD (start): hide elements of the GUI in "BloP-mode"
...
// SF: MOD (end): hide elements of the GUI in "BloP-mode"
}
    ...
    if (stage) {
        ...
        // SF: MOD: show custom blocks in the palette in alphabetical order
        stage.globalBlocks.sort(function(a, b){
			var compstr1 = a.type + "|" + a.spec,
				compstr2 = b.type + "|" + b.spec;
			if( compstr1 < compstr2) {
				return -1;
			} else if ( compstr1 > compstr2) {
				return 1;
			} else {
				return 0;
			}});
...
// SF: MOD: we don't add hidden custom blocks to the palette
if (!StageMorph.prototype.hiddenCustoms[block.blockSpec]) {
...
// SF: MOD: do not show usermenu for sprites on Stage in "BloP-mode"
if (!ide.isLocked) {
...
SpriteMorph.prototype.allHatBlocksFor = function (message) {
    ...
    return this.scripts.children.filter(function (morph) {
        ...
        if (morph.selector) {
            ...
            // SF: MOD: add "when load and never stop" block to Control category
            if (morph.selector === 'receiveOnLoad') {
                return message === '__run__never__stop__';
            }
            // SF: MOD: add "when run another script" block to Control category
            if (morph.selector === 'receiveOnRun') {
                return message === '__run__when__run__';
            }
...
// SF: MOD: empty hidden customs
StageMorph.prototype.hiddenCustoms = {};
// SF: MOD: to remove/add sprites/Stage from corral
StageMorph.prototype.hiddenObjects = {};
// SF: MOD: to remove/add categories
...
// SF: MOD: add "run on load and never stop" block to Control category
StageMorph.prototype.fireOnLoadEvent = function () {
    var procs = [],
        hats = [],
        ide = this.parentThatIsA(IDE_Morph),
        myself = this;
    ...
    this.children.concat(this).forEach(function (morph) {
        if (morph instanceof SpriteMorph || morph instanceof StageMorph) {
            hats = hats.concat(morph.allHatBlocksFor('__run__never__stop__'));
        }
    });
    hats.forEach(function (block) {
        procs.push(myself.threads.startProcess(
...
// SF: MOD: add "when run another script" block to Control category
StageMorph.prototype.fireOnRunEvent = function () {
    var procs = [],
        hats = [],
        ide = this.parentThatIsA(IDE_Morph),
        myself = this;
    ...
    this.children.concat(this).forEach(function (morph) {
        if (morph instanceof SpriteMorph || morph instanceof StageMorph) {
            hats = hats.concat(morph.allHatBlocksFor('__run__when__run__'));
        }
    });
    hats.forEach(function (block) {
        procs.push(myself.threads.startProcess(
...
// SF: MOD (start): allow to remove sprites from Corral if required by the user
if (!ide.isLocked) {
...
// SF: MOD (end): allow to remove sprites from Corral if required by the user
}
    return menu;
};
...
CellMorph.prototype.init = function (contents, color, idx, parentCell) {
    this.contents = (contents === 0 ? 0
            : contents === false ? false
                    : contents || '');
	// SF: MOD: makes cellmorphs non editable when GUI is locked
	// this.isEditable = isNil(idx) ? false : true;
    this.isEditable = world.children[0].isLocked ? false : (isNil(idx) ? false : true);
