...
IDE_Morph.prototype.createCategories = function () {
    ...
    function addCategoryButton(category) {
	    ...
	    // SF: MOD (start): allow to rename or hide a category
  	    if( !myself.isLocked) {
		    button.userMenu = function () {
				var menu = new MenuMorph();

				// SF: allow to rename a category
				menu.addItem(
					'rename...',
					function () {
					new DialogBoxMorph(
	    ...
	    // SF: MOD (end): allow to rename or hide a category
    ...
    // SF: MOD: show only visible categories at the TOP
...
// SF: MOD: show all elements in spritebat when GUI is not locked
if (!this.isLocked) {
...
// SF: MOD: hide elements in spritebar when GUI is locked
} else {
    thumbnail = new Morph();
    thumbnail.setExtent(thumbSize);
    thumbnail.image = this.currentSprite.thumbnail(thumbSize);
    ...
    // SF: OLD "bloP 2014 08 08" version
    // nameField.accept = function () {
	...
	// SF: NEW "Snap 2014 10 08" version
    nameField.accept = function () {
        var newName = nameField.getValue();
        myself.currentSprite.setName(
            myself.newSpriteName(newName, myself.currentSprite)
...
// SF: MOD (start): hide elements when GUI is locked
if (!this.isLocked) {
...
IDE_Morph.prototype.createSpriteEditor = function () {
    ...
    if (this.currentTab === 'scripts') {
        ...
        // SF: MOD: if the IDE is locked hide "when run/load" scripts...
		if( scripts && this.isLocked) {
			scripts.children.forEach(function (script) {
				if (script instanceof HatBlockMorph) {
					if (script.blockSpec == "when loading and never stop" || script.blockSpec == "when running another script") {
...
IDE_Morph.prototype.createCorralBar = function () {
    ...
    newbutton.drawNew();
	// SF: MOD: change elements when GUI is locked
...
// SF: MOD: IDE_Morph Locking/Unlocking
...
IDE_Morph.prototype.addNewSprite = function () {
	...
	// SF: MOD: in "bloP-mode" visible sprites get the "program" costume
	if( this.isLocked) {
		// SF: MOD: in "bloP-mode" visible sprites get the "program" name
		sprite.name = this.newSpriteName('Program');
		...
		// copy costume for new sprite (if any)
		if( spriteForImage.costume) {
...
IDE_Morph.prototype.removeSprite = function (sprite) {
    var idx, myself = this;
    // SF: MOD: detect if the sprite removed is the one currently visible
...
// SF: MOD: if the sprite removed is the one currently visible, look for the new current sprite
...
// SF: MOD: is the GUI is not locked
if (!this.isLocked) {
    ...
    menu.addItem(
    	...
    	// SF: MOD: show different labels when the GUI is locked/unlocked
        (this.isLocked) ? 'bloP website' : 'Snap! website',
        function () {

            // SF: MOD: go to Blop/Snap websites when the GUI is locked/unlocked
   ...
   // SF: MOD: allow to choose GUI behaviour
...
if (!this.isLocked) {
    ...
    menu.addItem(
	// SF: MOD: when GUI is locked the only possibility of opening a project is importing
    //  'Import...',
...
//SF: MOD (end): hide elements when GUI is locked
}
...
// SF: MOD (start): is the GUI is in "Snap-mode"
if (!this.isLocked) {
...
// SF: MOD (start): is the GUI is in "BloP-mode"
} else {
...
// SF: MOD (end): is the GUI is in "BloP-mode"
}
...
IDE_Morph.prototype.newProject = function () {
    ...
    // SF: MOD: the Stage is emptied only if the GUi is in "Snap-mode"
    // if (this.stage) {
...
// SF: MOD: the GUI is reset only if the GUi is in "Snap-mode"
if( !this.isLocked) {
...
IDE_Morph.prototype.rawSaveProject = function (name) {
    ...
    if (name) {
        ...
        if (Process.prototype.isCatchingErrors) {
			// SF: MOD: avoid tainted canvases (that cannot be exported)
			var thumbnail = this.stage.thumbnail(SnapSerializer.prototype.thumbnailSize),
				thumbdata;
			// catch cross-origin tainting exception when using SVG costumes
			try {
				thumbdata = thumbnail.toDataURL('image/png');
			} catch (error) {
                // SF: remove tainted canvases (that cannot be exported)
    ...
    if (name) {
        ...
        if (Process.prototype.isCatchingErrors) {
            ...
            try {
                ...
                // SF: MOD: update to save dialog from 2018/03/18
...
// SF: MOD: added from 2018/03/18 to add save dialog
...
IDE_Morph.prototype.saveFileAs = function (
    ...
    */
	...
	// SF: note: Blob API (https://www.javascripture.com/Blob) support (https://developer.mozilla.org/en-US/docs/Web/API/Blob)
    try {
    ...
    if (blobIsSupported) {
        ...
        // SF: note: saveAs from FileSaver.min.js
...
IDE_Morph.prototype.rawOpenProjectString = function (str) {
	...
	// SF: #### HERE WE EMPTY THE "HIDDEN" ARRAYS... BUT THIS FUNCTION IS CALLED WHENEVER THE GUI LOOK IS UPDATED!!!
    StageMorph.prototype.hiddenPrimitives = {};
...
function SpriteIconMorph(aSprite, aTemplate) {
    //SF: MOD: don't show sprite/Stage icon if it has been removed by user from corral
