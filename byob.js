...
CustomBlockDefinition.prototype.prototypeInstance = function () {
    ...
    // SF: each input slot ("part") has 4 (now 5) fragments that declares it: type (number, string, list, etc), default value, options (if any), readonly (if any) and option list (if any)
    block.parts().forEach(function (part) {
        if (part instanceof BlockInputFragmentMorph) {
            ...
            if (slot) {
                ...
                // SF: MOD: added option list when options are from a Snap list
...
// SF: return the default value of the "inputName" slot of the custom block
CustomBlockDefinition.prototype.defaultValueOf = function (inputName) {
	// SF: the default value is stored in the 1st element of the "declaration" of the slot (if any)
    if (this.declarations[inputName]) {
...
// SF: return the default value of the idx-th input slot of the custom block
...
// SF: return the options and the value of the readonly flag of the idx-th input slot of the custom block
CustomBlockDefinition.prototype.dropDownMenuOfInputIdx = function (idx) {
    var inputName = this.inputNames()[idx];
    return this.dropDownMenuOf(inputName);
};

// SF: check if the idx-th input slot is read only
CustomBlockDefinition.prototype.isReadOnlyInputIdx = function (idx) {
	// SF: look for the idx-th input slot
    var inputName = this.inputNames()[idx];
...
// SF: return a list "dict" of the values that can be selected in the menu
CustomBlockDefinition.prototype.dropDownMenuOf = function (inputName) {
    var dict = {};
	// SF: MOD: declarations[inputName][4] is the name of the option LIST (if any) associated with the "inputName" slot of the custom block
    // var ide = this.parentThatIsA(IDE_Morph);
    var ide = world.children[0];
    if (this.declarations[inputName] && this.declarations[inputName][4]) {
		// SF: vars can refer to objects and objects (blocks) can now refer to vars; so we defer the link between block inputs and var lists
	...
	// SF: declarations[inputName][2] is the list of options associated with the "inputName" slot of the custom block
    if (this.declarations[inputName] && this.declarations[inputName][2]) {
		// SF: splits declarations[inputName][2] at every newline (as edited in the dialog for creating options)
        this.declarations[inputName][2].split('\n').forEach(function (line) {
			// SF: if we used name/value pairs, split them at the "="
            var pair = line.split('=');
            // SF: the list of "dict" values is given by the the values pair[1] (if any) or the names pair[0]
...
// SF: return the readonly flag of the "inputName" slot
CustomBlockDefinition.prototype.isReadOnlyInput = function (inputName) {
	// SF: the readonly flag is stored in the 3rd element of the "declaration" of the slot (if any)
    return this.declarations[inputName] &&
        this.declarations[inputName][3] === true;
};

// SF: return the options and the value of the readonly flag of the "inputName" slot
...
// SF: return a list of all input slots of the custom block
...
CustomCommandBlockMorph.prototype.labelPart = function (spec) {
    ...
    // SF: ADD: flags for parts preeceding non-spacing-char/linkingchar '#'
	part.attachToNext = false;
...
// SF: var ide = this.parentThatIsA(IDE_Morph);
var ide = world.children[0];
...
// SF: MOD: do not show all items of user menu for custom (they are all custom!) blocks when ide is locked
// SF: TEMP: the test was disabled, it has been re-enabled for SNAPCON
if( !ide.isLocked) {
...
BlockLabelFragment.prototype.copy = function () {
	...
	// SF: MOD: handle list var for options (if any)
...
InputSlotDialogMorph.prototype.init = function (
    ...
    // SF: MOD: added 2 text height rows for text menus buttons in Input Slot Type dialog
...
InputSlotDialogMorph.prototype.getInput = function () {
    ...
    if (lbl) {
		...
		// SF: MOD: now we have two default input fields... (? see InputSlotDialogMorph.prototype.createSlotTypeButtons around line 2829)
        // this.fragment.defaultValue = this.slots.defaultInputField.getValue();
        this.fragment.defaultValue = this.slots.defaultInputMorph1.getValue();
        return lbl;
    }
    this.fragment.isDeleted = true;
    return null;
...
// SF: MOD: a lot different from the original one: NOTE: the new additions do not work (### IS THIS WHERE THE OK BUTTON DO NOT WORK?!?!??!)
InputSlotDialogMorph.prototype.createSlotTypeButtons = function () {
	...
	// SF: MOD: two more entries (to define menu entries) for the Input Slot Dialog
...
// SF: add a contextual menu to the "single value / default value" specification of input slots
// SF: the menu allow to define the menu options and if the slot is editable or not (read-only)
...
// SF: show a dialog where the user can type each menu option one per line (optionally defined as "option=value")
...
// SF: MOD: show a dialog where the user can select a list representing the input options
InputSlotDialogMorph.prototype.addSlotOptionsList = function () {
	...
	new DialogBoxMorph(
		...
		myself.world(),
		null, // pic
		ide.globalVariables.allNamesDict(), //
		true // read-only list
	);
