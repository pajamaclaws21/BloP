...
*/
    this.processes.forEach(function (proc) {
		// SF: MOD: added check on proc.homeContext
...
// SF: MOD: trigger "first" visible script of sprite
Process.prototype.runFirstScriptOfSprite = function (name) {
/// world.children[0].sprites.contents[0].scripts.children[0]
    var stage, sprite, firstScriptOfSprite;
    if (this.homeContext.receiver) {
        stage = this.homeContext.receiver.parentThatIsA(StageMorph);
        if (stage) {
			// SF: returns an array ==> need [0]
			sprite = stage.children.filter(function (morph) {
				return morph instanceof SpriteMorph && morph.name == name;
			});
			if(sprite) {
				// SF: returns an array ==> need [0]
				firstScriptOfSprite = sprite[0].scripts.children.filter(function (script) {
				    return script.isVisible;
			    });
				stage.threads.toggleProcess(firstScriptOfSprite[0]);
			}
        }
    }
};
