...
var propagatePasteEvent;
CursorMorph.prototype.processKeyDown = function (event) {
    // this.inspectKeyEvent(event);
    var shift = event.shiftKey;
    this.keyDownEventUsed = false;
    if (event.ctrlKey) {
		// SF: MOD: handle COPY & PASTE
		// SF: handle COPY
		if (event.keyCode == 67 || event.keyCode == 99) {
            var txt = this.target.text.substring( this.target.startMark, this.target.endMark);
            // SF: if explicitely requested by hitting SHIFT or if we try to copy twice the same text...
			if (event.shiftKey || world.clipboard == txt) {
                // SF: ... "external" copy
                window.prompt("To copy to clipboard OUTSIDE Snap: Ctrl+C then Enter\n(next time use Ctrl+Shift+C)", txt);
		    }
            // SF: "internal" copy
            world.clipboard = txt;
		...
		if (event.keyCode == 86 || event.keyCode == 118) {
            // SF: "internal" paste (no SHIFT)
			if (!event.shiftKey) {
                var txt = world.clipboard;
				if (txt && world.cursor) {
					world.cursor.insert(txt);
				}
				this.keyDownEventUsed = true;
				return;
            // SF: "external" paste (SHIFT)
		    } else {
				propagatePasteEvent = true;
		    }
        }
...
SliderMorph.prototype.drawNew = function () {
    ...
    if (this.orientation === 'vertical') {
        ...
        bh = this.height() - 2;
// SF: MOD: to avoid slider buttons as large as the slider in WatcherMorphs
if (typeof WatcherMorph != 'undefined' && this.parent instanceof WatcherMorph) {
	bw = bh;
} else {
...
// SF: MOD: IDEA: fare tutti i controlli sul timestamp DENTRO addEventListener!!!!!!!!!!!!!!!

    document.body.addEventListener(
        "paste",
        function (event) {
// SF: MOD: use standard clipboard
if (propagatePasteEvent) {
    ...
    // SF: MOD: end using standard clipboard
	propagatePasteEvent = false;
}
