...
XML_Serializer.prototype.format = function (string) {
    ...
    return string.replace(/[@$%]([\d]+)?/g, function (spec, index) {
        ...
        return spec === '@' ?	// when the placeholder is @ we escape all special chars, included QUOTES
                myself.escape(value)
                    : spec === '$' ?	// when the placeholder is $ we escape all special chars, except QUOTES
                        myself.escape(value, true)
                            : value;	// this latter is when the placeholder is %, without escaping
    });
};
...
// SF: MOD: do not allow to modify lists when GUI is locked
        ...
        if (inputs) {
            ...
            inputs.children.forEach(function (child) {
                ...
                definition.declarations[names[i]] = [
					...
					// SF: MOD: read name of option list var
...
StageMorph.prototype.toXML = function (serializer) {
	...
	// SF: MOD: the following line is useful to avoid tainted canvases (that cannot be exported)
	// SF: MOD: not necessary if we run a "clear" block before saving...
	// this.clearPenTrails();
    ...
    return serializer.format(
        ...
        Object.keys(this.declarations).reduce(function (xml, decl) {
                return xml + serializer.format(
					// SF: MOD: output name of option list var
                    // '<input type="@"$>$%</input>',
                    '<input type="@"$$>$%</input>',
